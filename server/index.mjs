...
app.get("/dich-vu", (req, res) => {
  renderPage(res, req.path, "pages/services", {
    ...
    pageDescription:
      "Tổng hợp đầy đủ dịch vụ AquaTech: website, app mobile, desktop software, scraping, automation và banner design.",
    pageKeywords: "dịch vụ công nghệ, web app, mobile app, automation, scraping",
  });
});

app.get("/bang-gia", (req, res) => {
  renderPage(res, req.path, "pages/pricing", {
    pricingCatalog,
    pageTitle: "Bảng giá | AquaTech",
    pageDescription:
      "Bảng giá cập nhật theo từng gói Basic, Standard, Advanced cho Desktop, Landing Page, Website Thương mại, Mobile, Scraping, Automation và dịch vụ khác.",
    pageKeywords:
      "bảng giá website, bảng giá app mobile, bảng giá phần mềm desktop, bảng giá scraping, bảng giá chrome automation",
  });
});

app.get("/pricing", (_req, res) => {
  res.redirect(301, "/bang-gia");
});
...
app.get("/bang-gia-tham-khao", (_req, res) => {
  res.redirect(301, "/bang-gia");
});
